...
export const MacbookScroll = ({ src, showGradient, title, badge }) => {
  ...
  return (
    <div
      ref={ref}
      className="md:h-[100vh] sm:h-[150vh] h-[100vh] flex flex-col items-center py-0 justify-start flex-shrink-0 [perspective:800px] transform md:scale-100  scale-50 sm:scale-75 sm:-translate-y-40 md:translate-y-0 -translate-y-40 sm:mb-96 md:mb-0"
    >
      <motion.h2
        style={{
      ...
      <div className="h-[22rem] w-[32rem] bg-[#272729] rounded-2xl overflow-hidden relative -z-10">
        ...
        {showGradient && <div className="h-40 w-full absolute bottom-0 inset-x-0 bg-gradient-to-t from-black  via-black to-transparent z-50"></div>}
...
export const Lid = ({ scaleX, scaleY, rotate, translate, src }) => {
  ...
  useEffect(() => {
    ...
    // GSAP Animation: Mengontrol opacity saat image mencapai bagian atas
    ...
    // Animasi mengubah opacity dari 1 ke 0
    tl.fromTo(image, { opacity: 1 }, { opacity: 0, ease: "power2.out" });

    return () => {
      if (tl) tl.kill(); // Clean up timeline on component unmount
    };
  }, []);
  return (
    <div className="relative [perspective:800px]">
      ...
      >
        ...
        <img src={src} alt="aceternity logo" className="object-cover object-left-top absolute rounded-lg inset-0 h-full w-full" />
      </motion.div>
    </div>
  );
};
...
export const KBtn = ({ className, children, childrenClassName, backlit = true }) => {
  return (
    <div className={cn("p-[0.5px] rounded-[4px]", backlit && "bg-white/[0.2] shadow-xl shadow-white")}>
      ...
      >
        <div className={cn("text-neutral-200 text-[5px] w-full flex justify-center items-center flex-col", childrenClassName, backlit && "text-white")}>{children}</div>
      </div>
    </div>
  );
};
